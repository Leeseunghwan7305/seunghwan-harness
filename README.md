...
4부품을 배우고 나서 실제로 쓰려고 붙인 커맨드들입니다.

### `/seunghwan-harness:commit-msg`
staged된 변경(`git add` 한 것)을 보고 커밋 메시지 초안을 만들어줍니다. **커밋은 안 하고 초안만** 보여줍니다.

### `/seunghwan-harness:split-commit`
이미 작업한 변경을 **기능 단위로 나눠** 여러 커밋으로 만들어줍니다. 커밋 계획을 먼저 보여주고, 승인하면 하나씩 커밋합니다. push는 안 합니다.

### `/seunghwan-harness:team <작업>`
하나의 작업을 **역할별 팀**에게 단계별로 넘겨 계획을 세웁니다. 실제 코드 수정·커밋은 **승인 후**에만.
...
**핵심 설계 두 가지:**
- 모든 역할이 `tools: [Read, Glob, Grep]` 만 가집니다 — **파일을 못 고칩니다.** 그래서 "계획까지만"이 구조적으로 보장됩니다.
- 매번 10명을 다 부르지 않습니다. 커맨드가 **작업에 맞는 역할만** 골라서 부르고, 누구를 왜 뺐는지 알려줍니다. 전원 소집은 `/seunghwan-harness:team --all <작업>`.

> 이 커맨드들이 **커맨드 + 에이전트 + (역할 위임)** 을 조합한 실제 예입니다. 4부품이 어떻게 맞물리는지 코드로 보고 싶으면 `commands/team.md` 와 `agents/*.md` 를 열어보세요.
